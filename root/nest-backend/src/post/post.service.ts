...
const TAG_EXTRACTOR_KEY =
  'eyJvcmciOiI2NTNiOTllNjEzOGM3YzAwMDE2MDM5NTEiLCJpZCI6IjUxMzA4NjZkNjgzZjRiNjk5MTVlYzBkMzlhMjJlYzA3IiwiaCI6Im11cm11cjEyOCJ9';
...
export class PostService {
  ...
  async createPost(dto: CreatePostDto) {
    const authorId: mongoose.Types.ObjectId = new mongoose.Types.ObjectId(
      dto.author,
    );
    let getKeyWords;
    try {
      getKeyWords = await axios.post(
        'https://gw.cortical.io/nlp/keywords',
        { text: dto.text },
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: TAG_EXTRACTOR_KEY,
          },
          params: { limit: 15 },
        },
      );
    ...
    const postTags: string[] = getKeyWords
      ? getKeyWords.data.keywords.map((obj) => obj.word)
      : [];
    ...
    if (dto.images.length !== 0) {
      post = new this.postModel({
        ...
        images: dto.images.map(
          (img: ImageDocument): mongoose.Types.ObjectId =>
            new mongoose.Types.ObjectId(img._id),
        ),
      });
      //-----update loaded images to change postId field as we already got it-----
      dto.images.forEach(async (img: ImageDocument): Promise<void> => {
  ...
  async getUserPosts(id: string, count: number) {
    ...
    const userPosts: PostDocument[] = await this.postModel
  ...
  async getRecommendations(id: string, postsOnThePage: string[]) {
    const postsOnThePageIds = postsOnThePage.map((id) => {
      return new mongoose.Types.ObjectId(id);
    });
    console.log(postsOnThePageIds);
    ...
    const friendsPosts = await Promise.all(
      user.friends.map(async (friend) => {
        ...
        const posts = userPosts.filter((post) => {
          if (
            !post.likes.some((userLike) => userLike === user._id) ||
            postsOnThePageIds.indexOf(post.id) === -1
          ) {
            /* || !post.views.some(userLike => userLike===user._id)*/
    ...
    const friendsLikes = user.friends.map(async (friend) => {
      ...
      return posts.map((post, indx) => {
        //filter these post so user doesn't receive old posts
        if (indx === 2) return;
        if (
          (!post.likes.some((userLike) => userLike === user._id) ||
            !post.views.some((userLike) => userLike === user._id)) &&
          isNotOutdated(14, post)
        )
          return post;
      });
    });
    ...
    function getPostsByKeyWords(): void {
      const keyWordsTypes = [
        {
          keyWordsType: 'frequentlyAppearingKeyWords',
          postsAmountByItsPriority: 5,
        },
        { keyWordsType: 'newKeyWords', postsAmountByItsPriority: 3 },
        { keyWordsType: 'oldKeyWords', postsAmountByItsPriority: 2 },
      ];
      keyWordsTypes.forEach(function ({
        keyWordsType,
        postsAmountByItsPriority,
      }) {
        requestPosts(keyWordsType, postsAmountByItsPriority);
      });
    }
    async function requestPosts(
      ...
      const posts: PostModel[] = await this.postModel.find({
        tags: [...userRecommendations[type]],
      }); //get all posts by key words type
      posts.filter(
        (post: PostModel) => postsOnThePageIds.indexOf(post._id) === -1,
      ); //filter posts that already on the page
    ...
    const newPosts: PostModel[] = await this.postModel
      .find(
        postsOnThePageIds.length === 0
          ? {}
          : { _id: { $nin: [...postsOnThePageIds] } },
      )
      .sort({ createdAt: -1 })
      .limit(15)
      .populate([
    ...
    //JUST FOR A TEST BECAUSE I NEED TO PROPERLY POPULATE REPLIES-----------------
    console.log('recently posts',newPosts.map((post) => post._id));
    newPosts.filter((post) => postsOnThePageIds.indexOf(post._id) === -1);
    recommendationPosts.push(...newPosts);

    //--------------------------recently uploaded posts--------------------------
    console.log(
      recommendationPosts.map((post) => post._id),
  ...
  async deletePost(id: string) {
    try {
      const isDeleted = await this.postModel
      ...
      if (!isDeleted)
        throw new NotFoundException({
          message:
            "Post wasn't deleted successfully, probably provided id is invalid",
        });
  ...
  async updatePost(id: string, dto: UpdatePostDto) {
    try {
      ...
      if (!post)
        throw new NotFoundException({
          message:
            "Post wasn't updated successfully, probably provided id is invalid",
        });
      ...
      if (dto.images.length !== 0) {
        ...
        const images = dto.images.map(
          (postImg) => new mongoose.Types.ObjectId(postImg._id),
        );
        oldPostImgs.forEach(async (img) => {
          //remove old images from db and from post itself
  ...
  async likePost(id: string, userId: string) {
    ...
    if (oldWords.length === 0) {
      ...
      if (newKeyWords.length === 0) {
        ...
        post.tags.forEach((tag) => {
          ...
          if (indx === -1) {
            ...
            user.recommendations.oldKeyWords = [
              ...user.recommendations.oldKeyWords,
              newKeyWords[indx],
            ];
          }
      ...
      Object.values(recomm._doc)
        .slice(0, 3)
        .forEach((category: []) => (oldWords = [...oldWords, ...category])); //{frequentlyAppearingKeyWords:[dogs,cats], newKeyWords:[politics], oldKeyWords:[food]} => [dogs,cats,politics,food]
      ...
      if (oldWords.length > 60) {
        ...
        user.recommendations = {
          frequentlyAppearingKeyWords: frequentlyAppearingWords,
          newKeyWords: newWords,
          oldKeyWords: oldRecommendations,
        };
      } else {
        user.recommendations = {
          frequentlyAppearingKeyWords: frequentlyAppearingWords,
          newKeyWords: newWords,
          oldKeyWords: oldWords,
        };
      }
    }
  ...
  async removeRecommendations(id: string, userId: string) {
    ...
    const updatedPost = await this.postModel.findByIdAndUpdate(postId, {
      removedRecommendation: { $push: userIdMongoose },
    });
