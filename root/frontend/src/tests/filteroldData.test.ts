...
describe('get age test', () => {
    it('getage', () => {
      expect(filterOldData(14, {createdAt:"2024-01-05T16:24:46.720+00:00"})).toBe(true)
    })
  })
...
//I need to find if the post is fresh
//freshness of the post/image is calculated by given number of days
//we need to find the difference between the number of days the post was created and the current days -> so if post 2024.01.05 and today is 2024.01.16 the difference is 11 days


function filterOldData(untilDaysNum:number/*num of days before data considered fresh*/,data:any) {
    const date = new Date();
