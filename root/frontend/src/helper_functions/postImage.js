export default function postImage (target, album, postId) {//saves image to 'imgbb.com' server
    ...
    rf.onload = async function (event) {
      ...
      fetch('https://api.imgbb.com/1/upload?key=432e8ddaeeb70d2d1be863e87c0f354e', { method: 'POST', body })
        .then((res) => res.json()).then((res) => {
          resolve({title: imgName.slice(0, imgName.lastIndexOf('.')), imageURL: res.data.url, postId, album})
        })
    }
  })
  return promise
